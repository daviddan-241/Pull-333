...
Flask keep-alive (for Render + UptimeRobot) + Telegram bot (polling),
one file, one process. No dummy fallbacks - everything is real.
...
Required env vars:
    ...
    SEED_PHRASE          - 12/24 BIP39 words (Phantom + MetaMask compatible)
Optional:
    SOLANA_RPC_URL       - default: https://api.mainnet-beta.solana.com
    HELIUS_RPC_URL       - enables full holder count
...
async def _ensure_funded(query, action_key: str, kb, extra_sol: float = 0.0) -> bool:
    ...
    addr = wallet.solana_pubkey
    text = (
        f"⚠️ *Wallet Not Funded*\n\n"
        f"This action needs at least `{needed:.4f}` SOL.\n"
        f"Current balance: `{bal:.4f}` SOL\n"
        f"Short by: `{max(needed - bal, 0):.4f}` SOL\n\n"
        f"💼 *Send SOL to:*\n`{addr}`\n\n"
        f"Make sure you fund this address on the *same Solana cluster* "
        f"the bot is using (mainnet vs devnet).\n\n"
...
async def router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "m_main":
        ...
        if not await _ensure_funded(query, "create_mint", launch_kb()):
            return
        ...
        if not mint:
            ...
            return
        if not await _ensure_funded(query, "mint", launch_kb()):
            return
        ...
        if not await _ensure_funded(query, "auto_launch", launch_kb()):
            return
        ...
        text = (
            ...
            f"Easiest way: import this seed into Phantom (Solana) or "
            f"MetaMask (EVM), then launch from there.\n\n"
            f"💼 *Solana wallet (use on Pump.fun):*\n`{addrs['solana']}`\n\n"
            f"💼 *EVM wallet (use on Four.meme):*\n`{addrs['ethereum']}`\n\n"
            f"After the token is live, copy the mint/contract and run:\n"
            f"`/settoken <mint>`\n\n"
            f"Then 🟢 Buy / 🔴 Sell / 📊 Analytics / 👥 Holders / 📈 Volume "
            f"will all work on it."
        )
        kb = InlineKeyboardMarkup([
        ...
        if not mint:
            await query.edit_message_text("❌ No token!", reply_markup=sell_kb())
            return
        if not await _ensure_funded(query, "sell", sell_kb()):
            return
        pct = int(d.split("_")[1])
        bal = trader.get_token_balance(mint)
        if bal["raw"] == 0:
        ...
        if not await _ensure_funded(query, "buy", buy_kb(), extra_sol=sol_amt):
            return
        ...
        if user_id in volume_engines and volume_engines[user_id].running:
            ...
            return
        if not await _ensure_funded(query, "volume", volume_kb()):
            return
